...
| **Rate limiter** | **`RateLimiterActor`** | **`atomr-infer-remote-core`** | **Cluster-distributed via `rustakka-distributed-data`** |
...
│   ├── atomr-infer-runtime/                actor implementations on rakka
...
│   │
│   ├── atomr-infer-pipeline/               rustakka-streams integration
│   ├── atomr-infer-testkit/                mock runtimes (local + remote), deterministic completion
│   ├── atomr-infer-cli/                    `rakka` binary
...
|---|---|---|
| `atomr-infer-core` | `serde`, `thiserror`, `bytes`, `secrecy` | tokio, async-trait, rakka, pyo3, runtime libs, http libs |
| `atomr-infer-runtime` | `atomr-infer-core`, rakka crates, tokio, async-trait, tracing | pyo3, runtime libs, http libs |
...
| `atomr-infer-pipeline` | core/runtime, `rustakka-streams` | runtime libs, pyo3 |
| `atomr-infer-testkit` | core/runtime, `rustakka-testkit`, `proptest`, `wiremock` (for remote mocks) | runtime libs, pyo3 |
...
| **Distributed rate limiting** | **new (v4, §3.5, §12.1)** | **`atomr-infer-remote-core` `RateLimiterActor` via `rustakka-distributed-data`** |
