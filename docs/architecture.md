...
- **Unified rate limiting and capacity management.** The same backpressure mechanism (bounded mailboxes, `atomr-streams`) handles "GPU saturated" and "OpenAI rate limit hit" with the same semantics propagated upstream.
...
### 2.1 vLLM V1 (canonical local LLM backend; `atomr-infer-runtime-vllm`)
...
### 2.2 TensorRT (`atomr-infer-runtime-tensorrt`)
...
Python kept where no Rust binding exists. `PythonGpuBridge` (CUDA §5.9, in `atomr-infer-python-bridge`) handles allocator and stream entry. Marked `runtime = "python:<name>"`.

### 2.7 Remote inference providers (`atomr-infer-runtime-openai`, `atomr-infer-runtime-anthropic`, `atomr-infer-runtime-gemini`, `atomr-infer-runtime-litellm`)
...
- **`RateLimiterActor`** holds the token bucket(s) for a `(provider, api_key, model)` tuple. Cluster-shared via `atomr-distributed-data` so multiple nodes coordinate.
...
LiteLLM is special: it's a proxy that exposes an OpenAI-compatible API in front of any backend. `atomr-infer-runtime-litellm` is implemented as a thin wrapper over `atomr-infer-runtime-openai` configured with a different endpoint URL. The distinction is preserved at the runtime level so operators can see "this is going through LiteLLM" vs "this is going direct to OpenAI" in deployment configs.
...
|---|---|---|---|---|---|
| `vllm` | `atomr-infer-runtime-vllm` | Python + Triton + PyTorch | Yes (`python-pinned`) | Local GPU | Python-pinned |
| `tensorrt` | `atomr-infer-runtime-tensorrt` | FFI to `libnvinfer.so` | No | Local GPU | Native GPU |
| `ort` | `atomr-infer-runtime-ort` | `ort` crate (CUDA EP) | No | Local GPU | Native GPU |
| `candle` | `atomr-infer-runtime-candle` | Pure Rust on `cudarc` | No | Local GPU | Native GPU |
| `mistralrs` | `atomr-infer-runtime-mistralrs` | Rust LLM runtime | No | Local GPU | Native GPU |
| `cudarc` | `atomr-infer-runtime-cudarc` | Direct kernel dispatch | No | Local GPU | Native GPU |
| `custom` | (third-party impl) | User-supplied `ModelRunner` | Depends | Local GPU or Remote | Depends |
| `python:<name>` | `atomr-infer-runtime-vllm` (scaffolding) | Python wrapper, no Rust path | Yes (`python-pinned`) | Local GPU | Python-pinned |
| **`openai`** | **`atomr-infer-runtime-openai`** | **Rust HTTP/2 client** | **No** | **Remote network** | **Standard tokio multi-threaded** |
| **`anthropic`** | **`atomr-infer-runtime-anthropic`** | **Rust HTTP/2 client** | **No** | **Remote network** | **Standard tokio multi-threaded** |
| **`gemini`** | **`atomr-infer-runtime-gemini`** | **Rust HTTP/2 client** | **No** | **Remote network** | **Standard tokio multi-threaded** |
| **`litellm`** | **`atomr-infer-runtime-litellm`** | **`atomr-infer-runtime-openai` re-targeted** | **No** | **Remote network** | **Standard tokio multi-threaded** |
...
The selection logic lives in `atomr-infer-core`'s `Deployment::infer_runtime()`. Operators can always override.
...
|---|---|---|---|
| API server | `ApiGatewayActor` | `atomr-infer-runtime` | Replaces FastAPI/Uvicorn or Triton's HTTP server |
| Per-request lifecycle | `RequestActor` | `atomr-infer-runtime` | One per active client request |
| DP coordinator | `DpCoordinatorActor` (cluster-singleton) | `atomr-infer-runtime` | Cluster-tools singleton with handover |
| Local engine core | `EngineCoreActor` | `atomr-infer-runtime` | Per-replica GPU orchestrator |
| **Remote engine core** | **`RemoteEngineCoreActor`** | **`atomr-infer-remote-core`** | **Per-replica HTTP orchestrator; queue + worker pool** |
| Scheduler / batching (local) | Module inside `EngineCoreActor` | runtime crate | NOT an actor |
| **Request queue (remote)** | **Module inside `RemoteEngineCoreActor`** | **`atomr-infer-remote-core`** | **Bounded priority queue** |
| KVCacheManager (vLLM) | Module inside `EngineCoreActor` | `atomr-infer-runtime-vllm` | LLM-specific |
| ModelExecutor | `ModelExecutorActor` | `atomr-infer-runtime` | Local-only; remote uses worker pool directly |
| Local worker | `WorkerActor` ≡ CUDA §3.1 `DeviceActor` | `atomr-infer-runtime` | Two-tier supervision for context poisoning |
| **Remote worker** | **`RemoteWorkerActor`** | **`atomr-infer-remote-core`** | **One per concurrent slot; HTTP client** |
| ModelRunner impl | `ModelRunnerActor` | runtime crate | Local: GPU kernels. Remote: HTTP request |
| Paged KV cache bytes | `PagedKvCacheActor` | `atomr-infer-runtime-vllm` | LLM-specific, local-only |
| **Rate limiter** | **`RateLimiterActor`** | **`atomr-infer-remote-core`** | **Cluster-distributed via `atomr-distributed-data`** |
| **Circuit breaker** | **`CircuitBreakerActor`** | **`atomr-infer-remote-core`** | **Per `(provider, endpoint)`** |
| **Remote session** | **`RemoteSessionActor`** | **`atomr-infer-remote-core`** | **Analog of `ContextActor`; handles credential refresh** |
| NCCL communicator | `CollectiveActor` (CUDA §3.3, §6.1) | `atomr-infer-runtime` | Local multi-GPU only |
| ZMQ / shared-memory IPC | atomr transport | `atomr-remote` (transitive) | One transport everywhere |

The pattern: **local-specific concerns (GPU, KV cache, NCCL) live in `atomr-infer-runtime` and per-runtime crates; remote-specific concerns (HTTP, rate limiting, circuit breakers) live in `atomr-infer-remote-core` and per-provider crates. Runtime-agnostic concerns (gateway, request actor, deployment manager, placement) are shared.**
...
```
                   ...
                   ApiGatewayActor                      runtime-agnostic
                            │ spawns one per request    (atomr-infer-runtime)
                            ▼
                    RequestActor
                            │   ask(routing target)
                            ▼
   ...
   └──────────────────────┘         │ uses:                      │
         ...
         ↑                                       ↑
   atomr-infer-runtime                atomr-infer-remote-core
   atomr-infer-runtime-vllm           atomr-infer-runtime-openai
   atomr-infer-runtime-tensorrt       atomr-infer-runtime-anthropic
   atomr-infer-runtime-ort            atomr-infer-runtime-gemini
   atomr-infer-runtime-candle         atomr-infer-runtime-litellm
...
vLLM's ZMQ + shared-memory + asyncio collapses to atomr transport. Remote runtimes use HTTP/2 over TLS for provider communication; this is application-protocol traffic, not IPC, and goes through standard `reqwest` or `hyper` clients held by `RemoteWorkerActor`.

### 5.6 Native local runtimes — the default path (unchanged)

Rust-native local runtimes run on standard atomr GPU dispatcher. No GIL, no allocator collision, CUDA-graph capture works directly.

### 5.7 Python local runtimes — `vllm` and the marked exception (unchanged)

`vllm` and `python:*` runtimes go through `atomr-infer-python-bridge`'s `PythonGpuBridge` on a `python-pinned` dispatcher. Default placement is dedicated-interpreter-per-deployment.
...
- **Cluster-distributed rate limiting.** `RateLimiterActor` syncs token-spent counters across nodes via `atomr-distributed-data` CRDT.
...
| Control plane loss | Cluster-tools singleton handover | Recover from `atomr-persistence` |
  ...
  template:
    spec:
      ...
      - name: engine
        ...
        command: ["atomr-infer", "serve", "--config", "/etc/gpt-4o.toml"]
        env:
        - { name: ATOMR_INFER_RUNTIME, value: "openai" }
...
The `EstimateCost` message is a typed query that returns predicted cost based on input length, model pricing, and expected output length. Built into `atomr-infer-runtime-{openai,anthropic,gemini,litellm}`; defaults to compute-cost-amortized for local deployments.
...
```
atomr-infer/                       (workspace root)
│
├── crates/
│   ├── atomr-infer-core/                   foundation: traits, types, no actor deps
│   ├── atomr-infer-runtime/                actor implementations on atomr
│   ├── atomr-infer-python-bridge/          Python interop (CUDA §5.9)
│   │
│   ├── atomr-infer-runtime-vllm/           vLLM (local, Python)
│   ├── atomr-infer-runtime-tensorrt/       TensorRT (local, Rust FFI)
│   ├── atomr-infer-runtime-ort/            ONNX Runtime (local)
│   ├── atomr-infer-runtime-candle/         Candle (local, pure Rust)
│   ├── atomr-infer-runtime-cudarc/         direct kernel dispatch (local)
│   ├── atomr-infer-runtime-mistralrs/      Rust LLM alternative (local)
│   │
│   ├── atomr-infer-remote-core/            shared HTTP / rate-limit / retry / circuit-breaker
│   ├── atomr-infer-runtime-openai/         OpenAI (+ Azure OpenAI variant)
│   ├── atomr-infer-runtime-anthropic/      Anthropic
│   ├── atomr-infer-runtime-gemini/         Google Gemini (Vertex + AI Studio)
│   ├── atomr-infer-runtime-litellm/        LiteLLM proxy (re-uses openai)
│   │
│   ├── atomr-infer-pipeline/               atomr-streams integration
│   ├── atomr-infer-testkit/                mock runtimes (local + remote), deterministic completion
│   ├── atomr-infer-cli/                    `atomr` binary
│   ├── atomr-infer-py-bindings/            PyO3 bridge for Python users
│   │
...
Eighteen crates total in v4 (up from fourteen in v3). The four added: `atomr-infer-remote-core`, `atomr-infer-runtime-openai`, `atomr-infer-runtime-anthropic`, `atomr-infer-runtime-gemini`, `atomr-infer-runtime-litellm`.

### 10.2 Dependency direction (extended)

```
atomr-infer-core         (leaf; no internal deps)
       ▲
       │
atomr-infer-runtime      ← atomr-infer-python-bridge      ← atomr-infer-remote-core
       ▲                       ▲                              ▲
       │                       │                              │
       │   ┌───────────────────┴────────────┐    ┌────────────┴──────────────┐
       │   │                                │    │                            │
atomr-infer-runtime-{tensorrt, ort,      atomr-infer-runtime-vllm    atomr-infer-runtime-{openai, anthropic, gemini}
   candle, cudarc, mistralrs}                                               ▲
                                                                            │
                                                                atomr-infer-runtime-litellm
       ▲                                                                    ▲
       │                                                                    │
atomr-infer-pipeline     atomr-infer-cli      atomr-infer-testkit ─────────────────┘
       ▲                       ▲                   ▲
       │                       │                   │
       └─────── inference (rollup) ────────────────┘
                       ▲
...
`atomr-infer-remote-core` is parallel to `atomr-infer-python-bridge`: a layer between `atomr-infer-runtime` and the per-runtime crates that need the shared infrastructure. Provider crates depend on `atomr-infer-remote-core`; the rollup doesn't see this directly (it only sees the per-runtime crates).

`atomr-infer-runtime-litellm` depends on `atomr-infer-runtime-openai` because LiteLLM exposes the OpenAI API; this is a thin re-targeting wrapper. Per the v3 rule about layering: this is the only intra-runtime dependency, and it's justified by API compatibility, not code reuse for its own sake.

### 10.3 Crate-by-crate description (additions)

**`atomr-infer-remote-core`** — shared remote infrastructure. Contains:
...
- `RateLimiterActor` (cluster-distributed via `atomr-distributed-data` GCounter; strict variant as cluster-singleton)
...
**`atomr-infer-runtime-openai`** — OpenAI Chat Completions API. Includes:
...
**`atomr-infer-runtime-anthropic`** — Anthropic Messages API. Includes:
...
**`atomr-infer-runtime-gemini`** — Google Gemini. Includes:
...
**`atomr-infer-runtime-litellm`** — LiteLLM proxy. Includes:
...
|---|---|---|
| `atomr-infer-core` | `serde`, `thiserror`, `bytes`, `secrecy` | tokio, async-trait, atomr, pyo3, runtime libs, http libs |
| `atomr-infer-runtime` | `atomr-infer-core`, atomr crates, tokio, async-trait, tracing | pyo3, runtime libs, http libs |
| `atomr-infer-python-bridge` | `atomr-infer-core`, `atomr-infer-runtime`, pyo3 | runtime libs, http libs |
| `atomr-infer-remote-core` | `atomr-infer-core`, `atomr-infer-runtime`, `reqwest`, `hyper`, `eventsource-stream`, `tower` | pyo3, GPU runtime libs |
| `atomr-infer-runtime-vllm` | core/runtime/python-bridge, pyo3 | other runtime libs |
| `atomr-infer-runtime-{tensorrt,ort,candle,cudarc,mistralrs}` | core/runtime, respective lib | pyo3, other runtime libs |
| `atomr-infer-runtime-{openai,anthropic,gemini}` | core/runtime/remote-core | pyo3, GPU runtime libs |
| `atomr-infer-runtime-litellm` | core/runtime/remote-core/openai | pyo3, GPU runtime libs |
| `atomr-infer-pipeline` | core/runtime, `atomr-streams` | runtime libs, pyo3 |
| `atomr-infer-testkit` | core/runtime, `atomr-testkit`, `proptest`, `wiremock` (for remote mocks) | runtime libs, pyo3 |
| `inference` (rollup) | all above optionally | nothing forbidden |

The `secrecy` crate is added to `atomr-infer-core` because credentials need to be a part of the type system from the bottom up (deployments declare API keys; the type prevents accidental logging).
...
candle    = ["dep:atomr-infer-runtime-candle",    "atomr-infer-runtime-candle/candle"]
cudarc    = ["dep:atomr-infer-runtime-cudarc",    "atomr-infer-runtime-cudarc/cudarc"]
...
litellm   = ["dep:atomr-infer-runtime-litellm"]
...
`atomr-infer-core` and `atomr-infer-runtime` version slowly. Per-runtime crates version independently. Rollup is the synchronization point.
...
atomr-infer-remote-core         = { version = "0.1.0", path = "crates/atomr-infer-remote-core" }
atomr-infer-runtime-openai      = { version = "0.1.0", path = "crates/atomr-infer-runtime-openai" }
atomr-infer-runtime-anthropic   = { version = "0.1.0", path = "crates/atomr-infer-runtime-anthropic" }
atomr-infer-runtime-gemini      = { version = "0.1.0", path = "crates/atomr-infer-runtime-gemini" }
atomr-infer-runtime-litellm     = { version = "0.1.0", path = "crates/atomr-infer-runtime-litellm" }
```
...
- **Third-party remote providers are first-class.** A custom provider (Cohere, Mistral API, AWS Bedrock, internal LLM gateway) is implemented by depending on `atomr-infer-remote-core` + `atomr-infer-core` and producing a `MyProviderRunner: ModelRunner`. No fork, no vendor.
...
endpoint = "atomr://controlplane.prod:7355"
...
|---|---|---|
| Thread affinity (local) | §5.1 | `atomr-infer-runtime` `WorkerActor` dispatcher |
| Asynchrony (local) | §5.2, §5.10 | `atomr-infer-core` `ModelRunner::execute` |
| Local supervision | §5.3, §5.11 | `atomr-infer-runtime` two-tier; per-runtime crate rebuild logic |
| Backpressure | §5.4 | `atomr-infer-runtime` bounded mailboxes; `atomr-infer-pipeline` for streams |
| Serialization / location transparency | §5.5, §5.8 | `atomr-infer-core` `GpuRef<T>` / `GpuToken<T>` |
| Memory transfer (local) | §5.6 | `atomr-infer-runtime` placement; per-runtime allocator hooks |
| Stream allocation (local) | §5.7 | `atomr-infer-runtime` `StreamAllocator` |
| `GpuRef<T>` lifecycle | §5.8 | `atomr-infer-core` types |
| Python GPU bridge | §5.9 | `atomr-infer-python-bridge` |
| Completion strategy (local) | §5.10 | `atomr-infer-runtime` `HostFnCompletion` |
| Two-tier supervision | §5.11 | `atomr-infer-runtime`; per-runtime rebuild |
| Runtime selection and GIL exposure | new (v2) | `atomr-infer-core` types; `atomr-infer-runtime` `PlacementActor` |
| Pipeline composition backpressure | new (v2) | `atomr-infer-pipeline` |
| Selective compilation | new (v3) | Feature flags forward through `inference` rollup |
| **Distributed rate limiting** | **new (v4, §3.5, §12.1)** | **`atomr-infer-remote-core` `RateLimiterActor` via `atomr-distributed-data`** |
| **Provider failure isolation** | **new (v4, §3.5, §12.2)** | **`atomr-infer-remote-core` `CircuitBreakerActor`** |
| **Remote retry semantics** | **new (v4, §3.5, §12.3)** | **`atomr-infer-remote-core` retry policy module** |
| **Cost tracking and budgets** | **new (v4, §12.4)** | **`MetricsActor` aggregation; budget enforcement in `RemoteEngineCoreActor`** |
| **Credential lifecycle** | **new (v4, §12.5)** | **`RemoteSessionActor`; `secrecy` types in `atomr-infer-core`** |
...
- Provider published pricing (in `atomr-infer-runtime-{openai,anthropic,gemini}` defaults; overridable)
...
In parallel with 2a/2b. Remote runtimes don't depend on Python bridge or local runtime crates beyond `atomr-infer-core` and `atomr-infer-runtime`.

1. `atomr-infer-remote-core` complete:
...
- `atomr-infer cost-report` CLI subcommand
- `atomr-infer rotate-credentials` CLI subcommand
- Provider compatibility matrix in `defaults.toml`

### Phase 7 — Out-of-scope-but-watch (extended)

- Additional remote providers: AWS Bedrock, Cohere, Mistral API, Together AI — community-contributed `atomr-infer-runtime-*` crates.
...
| What's the workspace story? | 18 crates, strict layering. `atomr-infer-core` no actor deps; `atomr-infer-remote-core` parallel to `atomr-infer-python-bridge`; per-provider crates plug in. Pure-remote builds compile no GPU deps. |
...
Workspace structure; 14 crates; strict one-way layering; `atomr-infer-python-bridge` separated; `atomr-infer-testkit` first-class; bare `inference` rollup.
...
| 14 crates | 18 crates (added `atomr-infer-remote-core`, `atomr-infer-runtime-openai`, `atomr-infer-runtime-anthropic`, `atomr-infer-runtime-gemini`, `atomr-infer-runtime-litellm`) |
