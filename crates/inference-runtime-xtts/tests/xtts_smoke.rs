...
//! Requires:
//!
//! - the `tts-xtts` feature (`cargo test -p atomr-infer-runtime-xtts
//!   --features tts-xtts --test xtts_smoke -- --ignored`),
//! - `XTTS_MODEL_PATH` pointing at an XTTS-v2 ONNX model directory
...
use atomr_infer_runtime_xtts::{XttsConfig, XttsRunner};
use futures::StreamExt;

#[tokio::test]
#[ignore = "needs XTTS_MODEL_PATH + a real XTTS model on disk"]
async fn synth_returns_pcm_with_terminal_chunk() {
    let model_path = match std::env::var("XTTS_MODEL_PATH") {
        Ok(v) => PathBuf::from(v),
        Err(_) => {
            eprintln!("skipping: set XTTS_MODEL_PATH to run");
            return;
        }
    };

    let mut runner = XttsRunner::new(XttsConfig {
        model_path,
