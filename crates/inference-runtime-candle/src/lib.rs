...
//! [`rakka_cuda::dispatcher::GpuDispatcher`] for thread pinning and
//! [`rakka_cuda::stream::PerActorAllocator`] for per-request stream
...
impl ModelRunner for CandleRunner {
    async fn execute(&mut self, _batch: ExecuteBatch) -> InferenceResult<RunHandle> {
        ...
        #[cfg(feature = "candle")]
        {
            ...
            let _ = (); // ensure rakka_cuda is referenced once the body lands
            return Err(InferenceError::Internal(
                "candle runner: forward pass pending — wire via \
                 rakka_cuda::dispatcher::GpuDispatcher (doc §13 Phase 2b)"
                    .into(),
            ));
        }
    }
