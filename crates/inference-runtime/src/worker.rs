...
//! [`rakka_cuda::error::CONTEXT_POISONED_TAG`] marker so that
//! [`rakka_cuda::error::device_supervisor_strategy`] routes the
//! failure to `Directive::Restart`.
//!
//! The supervision *policy* (3 retries / 60s, decider, marker tags) is
//! re-used verbatim from rakka-cuda's `error` module — that's the
...
/// `rakka_cuda::device::DeviceState`, or whatever the backend uses);
...
impl Actor for WorkerActor {
    ...
    fn supervisor_strategy(&self) -> SupervisorStrategy {
        ...
        // don't pull rakka-cuda but still happen to mount a local
        // ModelRunner (e.g. inference-testkit's MockRunner in tests).
        #[cfg(feature = "local-gpu")]
        {
            rakka_cuda::error::device_supervisor_strategy()
        }
        #[cfg(not(feature = "local-gpu"))]
        {
            ...
            OneForOneStrategy::new()
                ...
                .with_decider(|err| {
                    // Mirror rakka_cuda::error::decider's tag set.
...
/// `rakka_cuda::device::ContextActor`: that one specialises to CUDA
                ...
                tokio::spawn(async move {
                    ...
                    match g.execute(batch).await {
                        ...
                        Err(e) => {
                            ...
                            // tagged with rakka_cuda's CONTEXT_POISONED_TAG
                            ...
                            if matches!(e, InferenceError::CudaContextPoisoned(_)) {
                                ...
                                panic!(
                                    "{}: {e}",
                                    rakka_cuda::error::CONTEXT_POISONED_TAG
                                );
                                #[cfg(not(feature = "local-gpu"))]
