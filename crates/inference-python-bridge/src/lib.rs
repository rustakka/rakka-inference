...
//! crate. As of the atomr-accel F1–F5 surfaces, the upstream crate
//! still notes the `PythonGpuBridge` as F4-deferred (see
//! `atomr-accel/src/lib.rs` header). We implement it here in the
//! meantime. When atomr-accel exposes its own `PythonGpuBridge`, this
//! crate switches to a re-export — see `bridge.rs` for the lift
//! marker (`TODO(atomr-accel F4)`).
//!
